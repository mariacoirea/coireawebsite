    ...
    <section id="services" className="relative py-24 bg-gradient-to-br from-warm-beige to-aura-pearl overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="text-center mb-12 sm:mb-16 lg:mb-20">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display font-semibold text-primary mb-4 sm:mb-6 leading-tight px-2">
            Transformational Offerings for Regenerative Growth
          </h2>
          
          <p className="text-lg sm:text-xl text-muted-foreground font-body max-w-3xl mx-auto mb-6 sm:mb-8 px-2">
            We co-create regenerative pathways tailored to your organization's unique rhythm and evolution.
          </p>

          <p className="text-base sm:text-lg text-muted-foreground/80 font-body max-w-4xl mx-auto mb-6 sm:mb-8 px-2">
          ...
          <div className="w-16 sm:w-24 h-0.5 bg-gradient-warm mx-auto"></div>
        </div>

        {/* Tier 1 - Core Offering (Full Width) */}
        <div className="mb-12 sm:mb-16 max-w-6xl mx-auto">
          <Card className="p-4 sm:p-6 lg:p-10 hover:shadow-elegant transition-all duration-500 group border-0 bg-card/60 backdrop-blur-sm hover:scale-[1.01] relative">
            {/* Signature Badge */}
            <div className="absolute top-3 right-3 sm:top-4 sm:right-4 lg:top-6 lg:right-6">
              <div className="flex items-center space-x-1 sm:space-x-2 bg-accent/15 px-2 sm:px-3 lg:px-4 py-1 sm:py-2 rounded-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-start">
              <div>
                {/* Header */}
                <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-4 sm:mb-6 pt-8 sm:pt-0">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-hero rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 mx-auto sm:mx-0">
                    <coreOffering.icon className="w-8 h-8 sm:w-10 sm:h-10 text-primary-foreground" />
                  </div>
                  <div className="text-center sm:text-left">
                    <h3 className="text-xl sm:text-2xl lg:text-3xl font-display font-semibold text-primary group-hover:text-primary-light transition-colors leading-tight">
                      {coreOffering.title}
                    </h3>
                    <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 mt-2 sm:mt-3">
                      <span className="text-xs sm:text-sm text-accent font-medium bg-accent/10 px-3 py-1 rounded-full inline-block">
                        {coreOffering.type}
                      </span>
                ...
                {/* Description */}
                <p className="text-muted-foreground font-body leading-relaxed mb-4 sm:mb-6 text-base sm:text-lg text-center sm:text-left">
              ...
              <div>
                {/* Features */}
                <div>
                  <h4 className="text-sm font-body font-semibold text-primary mb-3 sm:mb-4 uppercase tracking-wide text-center sm:text-left">
                    Key Areas
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                    {coreOffering.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center space-x-3 justify-center sm:justify-start">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-6xl mx-auto mb-12 sm:mb-16">
          {shortTermOfferings.map((service, index) => (
            <Card 
              key={index} 
              className="p-4 sm:p-6 lg:p-8 hover:shadow-elegant transition-all duration-500 group border-0 bg-card/60 backdrop-blur-sm hover:scale-[1.02]"
            >
              {/* Header */}
              <div className="mb-4 sm:mb-6">
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-hero rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 mb-3 sm:mb-4 mx-auto sm:mx-0">
                  <service.icon className="w-6 h-6 sm:w-8 sm:h-8 text-primary-foreground" />
                </div>
                <h3 className="text-lg sm:text-xl font-display font-semibold text-primary group-hover:text-primary-light transition-colors mb-2 sm:mb-3 text-center sm:text-left leading-tight">
                  {service.title}
                </h3>
                <div className="flex flex-col space-y-2 items-center sm:items-start">
                  <span className="text-xs sm:text-sm text-accent font-medium bg-accent/10 px-2 sm:px-3 py-1 rounded-full inline-block">
                    {service.type}
                  </span>
                  <span className="text-xs sm:text-sm text-muted-foreground">
                    {service.duration}
              ...
              {/* Description */}
              <p className="text-muted-foreground font-body leading-relaxed mb-4 sm:mb-6 text-sm sm:text-base text-center sm:text-left">
                {service.description}
              </p>

              {/* Features */}
              <div className="mb-4 sm:mb-6">
                <h4 className="text-xs sm:text-sm font-body font-semibold text-primary mb-2 sm:mb-3 uppercase tracking-wide text-center sm:text-left">
                  Key Areas
                </h4>
                <div className="space-y-2">
                  {service.features.map((feature, idx) => (
                    <div key={idx} className="flex items-center space-x-2 justify-center sm:justify-start">
        ...
        <div className="text-center px-2">
          <div className="max-w-3xl mx-auto mb-6 sm:mb-8">
            <p className="text-base sm:text-lg text-muted-foreground font-body leading-relaxed mb-3 sm:mb-4">
          ...
          <Link to="/journey">
            <Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground group px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base min-h-[48px] touch-manipulation">
              Begin Your Journey
